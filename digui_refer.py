...
class Recursion(object):
    ...
    def geq_end_if(self):
        quaternion = MiddleCode("ei", None, None, None)
        self.midCodeRes.append(quaternion)
    ...
    def geq_end_while(self):
        quaternion = MiddleCode("ew", None, None, None)
        self.midCodeRes.append(quaternion)
    ...
    def geq_call_function(self, function):  #
        result = "t" + str(self.count)
        self.count += 1
        quaternion = MiddleCode("call", function, None, result)
        self.midCodeRes.append(quaternion)
        self.SEMStack.push1(result)
    ...
    def operation_object(self):
        if (self.jud_fun()):
            ...
            if (self.token_to_word() == '('):
                self.get_next_token()
                self.with_or_without_parameters_1()
                self.geq_call_function(function_name)
                return
            else:
                self.error()
        elif (self.jud_nfun()):
